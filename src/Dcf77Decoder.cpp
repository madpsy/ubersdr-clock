...
struct Dcf77Decoder::Impl {
    ...
    void pmAcquireStep() {
        ...
        const int64_t off0 = clockdec::floorMod(lo - acqBase, pint);
        ...
        const int64_t tau0 = lo + clockdec::floorMod(bestIdx - off0, pint);
    ...
    Decoded decode(Src src) const {
        ...
        const long long days = clockdec::daysFromCivil(2000 + d.year2, static_cast<unsigned>(d.month),
        ...
        clockdec::civilFromDays(days, cy, cm, cd);
        if (static_cast<int>(cm) != d.month || static_cast<int>(cd) != d.day) return d;
        if (static_cast<int>(clockdec::floorMod(days + 3, 7)) + 1 != wday) return d;
        ...
        d.utc = clockdec::hostNowFields(d.utcMs);
        d.ok = true;
        return d;
    }
    ...
    static bool leapSecondPossible(const TimeFields& f) {
        ...
        return clockdec::isLastDayOfMonth(clockdec::utcMsFromFields(f.year2, f.doy, f.hour, f.minute));
    }
